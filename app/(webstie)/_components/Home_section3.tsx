...
import { get_homePage_trendings } from "@/lib/api";
...
export default function Home_section3() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                const data = await get_homePage_trendings();
                setProducts(Array.isArray(data) ? data : []);
            } catch (error) {
                console.error("Error fetching the trendings", error);
                setErrorMessage("Unable to load trending products right now.");
            } finally {
                setIsLoading(false);
            }
        };
        ...
        <section className="bg-white py-12 md:py-14">
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between gap-4 mb-4">
                    <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-500">Trendings</p>
